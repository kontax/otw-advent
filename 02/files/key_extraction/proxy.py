...
input(f"Visit http://3.93.128.89:12020/ and enter {server_id.decode()}")
...
with open('login', 'r') as f:
    login_pt = bytes.fromhex(f.read())

with open('response', 'r') as f:
    resp_pt = bytes.fromhex(f.read())
...
while True:
    ...
    # Encode the login text with the key we already have, and send it to the server
    encoded = []
    for b in login_pt:
    ...
    print("\n----- CLIENT -----")
    print(hexdump.hexdump(bytes(encoded)))
    server.send(bytes(encoded))

    # Get the data back from the server, decoding what we know with the key we
    # have, and extracting the key from the rest
    print("\n----- SERVER -----")
    buf = server.recv(2)
    time.sleep(0.25)
    buf = buf + server.recv(BUFFER)
    print(hexdump.hexdump(buf))
    new_key = []
    for b in buf:
        if SRV_IDX < len(resp_pt):
            new_key.append(KEY[SRV_IDX])
        else:
            key_byte = b ^ resp_pt[SRV_IDX % len(resp_pt)] ^ init[SRV_IDX % len(init)]
            new_key.append(key_byte)
            KEY.append(key_byte)
    ...
    time.sleep(5 if i % 10 == 0 else 0.2)
    i += 1
